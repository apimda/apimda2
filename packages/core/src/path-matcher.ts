export class PathMatcher {
  static parse(path: string) {
    ...
    return new PathMatcher(path, normalizedPath, normalizedComponents, pathVars);
  }
   ...
   * @param requestPath
   */
  match(requestPath: string): Map<string, string> | undefined {
    ...
    for (let i = 0; i < this.normalizedComponents.length; i++) {
      ...
      if (normalizedComponent === '{}') {
        ...
        result.set(pathVarName, requestComponent);
